...
if not os.path.exists(inventory_file):
    ...
    df = pd.DataFrame(columns=["Computer Name", "Serial Number", "CPU", "RAM", "Storage"])
    df.to_csv(inventory_file, index=False)

def add_computer(computer_name, serial_number, cpu, ram, storage):
...
def main():
    while True:
        ...
        if choice == "1":
            ...
            add_computer(computer_name, serial_number, cpu, ram, storage)
        elif choice == "2":
            display_inventory()
        elif choice == "3":
